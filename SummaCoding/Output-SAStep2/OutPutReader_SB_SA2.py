...
output_sb_Ac_ASl_SWRc_TCj = Dataset("Ac_ASlouis_SWRclm_TCSjrdn_2010-2011_senatorConstantDecayRate_1.nc")

for i in output_sb_Ac_ASl_SWRc_TCj.variables:
    print i

hru_num = np.size(output_sb_Ac_ASl_SWRc_TCj.variables['hru'][:])

#%% output time step
TimeSb = output_sb_Ac_ASl_SWRc_TCj.variables['time'][:] # get values
t_unitSb = output_sb_Ac_ASl_SWRc_TCj.variables['time'].units # get unit  "days since 1950-01-01T00:00:00Z"

try :

    t_cal = output_sb_Ac_ASl_SWRc_TCj.variables['time'].calendar
...
out1_df_int = np.column_stack([output_sb_Ac_ASl_SWRc_TCj['scalarSnowDepth'][:,0],output_sb_Ac_ASl_SWRc_TCj['scalarSurfaceTemp'][:,0],output_sb_Ac_ASl_SWRc_TCj['scalarSenHeatTotal'][:,0],output_sb_Ac_ASl_SWRc_TCj['scalarLatHeatTotal'][:,0]])

snowdepth_Ac_ASl_SWRc_TCj = output_sb_Ac_ASl_SWRc_TCj.variables['scalarSnowDepth'][:]
hru = ['hru{}'.format(i) for i in range(1, hru_num+1)]
snowdepth_Ac_ASl_SWRc_TCj_df = pd.DataFrame (snowdepth_Ac_ASl_SWRc_TCj, columns=[hru])
snowdepth_Ac_ASl_SWRc_TCj_df.set_index(pd.DatetimeIndex(DateSb),inplace=True)

counter = pd.DataFrame(np.arange(0,np.size(snowdepth_Ac_ASl_SWRc_TCj_df['hru1'])),columns=['counter'])
counter.set_index(snowdepth_Ac_ASl_SWRc_TCj_df.index,inplace=True)
snowdepth_Ac_ASl_SWRc_TCj_df2 = pd.concat([counter, snowdepth_Ac_ASl_SWRc_TCj_df], axis=1)

#%%   day of snow disappearance-output ---- Sensititvity analysis - Step2
snowdepth_Ac_ASl_SWRc_TCj_df6000 = snowdepth_Ac_ASl_SWRc_TCj_df2[:][6000:6553]

zerosnowdate = []
for val in hru:
    zerosnowdate.append(np.where(snowdepth_Ac_ASl_SWRc_TCj_df6000[val]==0)[0])
...
for hruname in hru:
    ...
    plt.plot(sbx, snowdepth_Ac_ASl_SWRc_TCj_df[hruname], sbx, sd_obs2010, 'k--', linewidth=0.5)#, label='wwe', color='maroon') param_nam_list[q] color_list[q]
    plt.title('Aconst_ASlouis_SWRclm_TCjrdn', position=(0.04, 0.88), ha='left', fontsize=12)
    plt.xlabel('Time 2010-2011')
    plt.ylabel('Snow Depth (m)')
#plt.show()
plt.savefig('SnowDepth_Ac_ASl_SWRc_TCj.png')
#%% r-squered and RMSE for snowdepth 
r2sd_Ac_ASl_SWRc_TCj =[]
for item in hru:
    slope, intercept, r_value, p_value, std_err = stats.linregress(sd_obs2010,snowdepth_Ac_ASl_SWRc_TCj_df2[item])
    r2sd_Ac_ASl_SWRc_TCj.append(np.power(r_value,2))

rmsesd_Ac_ASl_SWRc_TCj =[]
for item in hru:
    rmsesd_Ac_ASl_SWRc_TCj.append(np.power(mean_squared_error(sd_obs2010,snowdepth_Ac_ASl_SWRc_TCj_df2[item]),0.5))

plt.plot(rmsesd_Ac_ASl_SWRc_TCj, r2sd_Ac_ASl_SWRc_TCj, 'ro')
plt.title('snowDepth_Aconst_ASlouis_SWRclm_TCjrdn', position=(0.04, 0.03), ha='left', fontsize=12)
plt.xlabel('RMSE')
plt.ylabel('R2')
plt.savefig('RMSE-R2-SnowDepth_Ac_ASl_SWRc_TCj.png')
#%% skinTemp dataframe
skinTemp_Ac_ASl_SWRc_TCj = output_sb_Ac_ASl_SWRc_TCj.variables['scalarSurfaceTemp'][:]
skinTemp_Ac_ASl_SWRc_TCj_df = pd.DataFrame (skinTemp_Ac_ASl_SWRc_TCj, columns=[hru])
skinTemp_Ac_ASl_SWRc_TCj_df.set_index(pd.DatetimeIndex(DateSb),inplace=True)
skinTemp_Ac_ASl_SWRc_TCj_df2 = skinTemp_Ac_ASl_SWRc_TCj_df-273.15
...
r2st_Ac_ASl_SWRc_TCj =[]
for item in hru:
    slope, intercept, r_value, p_value, std_err = stats.linregress(skintemp2010,skinTemp_Ac_ASl_SWRc_TCj_df2[item])
    r2st_Ac_ASl_SWRc_TCj.append(np.power(r_value,2))

rmsest_Ac_ASl_SWRc_TCj =[]
for item in hru:
    rmsest_Ac_ASl_SWRc_TCj.append(np.power(mean_squared_error(skintemp2010,skinTemp_Ac_ASl_SWRc_TCj_df2[item]),0.5))

plt.plot(rmsest_Ac_ASl_SWRc_TCj, r2st_Ac_ASl_SWRc_TCj, 'bo')
plt.title('skinTemp_Aconst_ASlouis_SWRclm_TCjrdn', position=(0.04, 0.03), ha='left', fontsize=12)
plt.xlabel('RMSE')
plt.ylabel('R2')
plt.savefig('RMSE_R2_skinTemp_Ac_ASl_SWRc_TCj.png')
