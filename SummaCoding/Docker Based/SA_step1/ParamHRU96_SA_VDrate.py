...
p1 = [0.8, 0.90] #albedoMax      |       0.8400 |       0.7000 |       0.9500 $$$$$$ 2: 0.75 | 0.92
p2 = [0.45, 0.55] #albedoMinWinter   |       0.5500 |       0.6000 |       1.0000 $$$$$$ 2: 0.65 | 0.90
...
p6 = [1000000, 2000000, 4000000] #albedoDecayRate  |       1.0d+6 |       0.1d+6 |       5.0d+6 $$$$$$ 3: 0.8d+6 | 2d+6 | 4.0d+6
...
print np.size(valst1)
#%% #create new paramtrail.nc file and adding vaiables to it
#"settings/SH/t1paramtrial_36h.nc"
paramfile = Dataset("summa_zParamTrial_96hru_V.nc",'w',format='NETCDF3_CLASSIC') #create new paramtrail.nc file
...
#%% # add values for the changing variables in 96 HRUs
j = 0 
...
#varcheck = Dataset ('summa_zParamTrial_96hru.nc')
...
local_atrbt = Dataset("LocalAtributes96hru.nc",'w',format='NETCDF3_CLASSIC')
# define dimensions 
hru = local_atrbt.createDimension('hru', 96) 
...
c3 = np.repeat(newgru[:,np.newaxis], 96, axis=1); newlad = c3.reshape(96,)
...
local_atrbt.close()
#%%
laCheck = Dataset('LocalAtributes96hru.nc')
...
in_condi = Dataset("InitialCond96hru.nc",'w',format='NETCDF3_CLASSIC')
#print ic.variables.keys()
...
hrud = in_condi.createDimension('hru', 96)
# define variables
...
#%% # add values for the intial condition variables in 96 HRUs
...
for varname in icif.variables.keys():
    ...
    cic = np.repeat(var[:,np.newaxis], 96, axis=1); newic = cic.reshape(infovar.shape[0],96)
    in_condi.variables[varname][:]=newic
#print in_condi.variables['nSoil'][:]

in_condi.close()
#%%
iccheck = Dataset("InitialCond96hru.nc")
