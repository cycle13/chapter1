...
p1 = [0.77, 0.86, 0.91] #albedoMax      |       0.8400 |       0.7000 |       0.9500 $$$$$$ 2: 0.75 | 0.92
...
p6 = [0.8, 0.93] #albedoMaxVisible          |       0.9500 |       0.7000 |       0.9500 ****** 1: 0.95
p7 = [0.6, 0.73] #albedoMinVisible          |       0.7500 |       0.5000 |       0.7500 ****** 1: 0.75

def hru_ix_ID(p1, p2, p3, p4, p5, p6, p7):
    ...
    c = list(itertools.product(ix1,ix2,ix3,ix4,ix5,ix6,ix7))
    ix_numlist=[]
    for tup in c:
...
hruidxID = hru_ix_ID(p1, p2, p3, p4, p5, p6, p7)
# %store hruidxID
#%%
param_nam_list = ['albedoMax', 'albedoMinWinter', 'albedoMinSpring', 'albedoMaxNearIR', 'albedoMinNearIR', 'albedoMaxVisible', 'albedoMinVisible'] 
# function to create lists of each parameter, this will iterate through to make sure all combinations are covered
def param_fill(p1, p2, p3, p4, p5, p6, p7): 
    b = list(itertools.product(p1, p2, p3, p4, p5, p6, p7)) 
    p1l =[]; p2l =[]; p3l =[]; p4l=[]; p5l = []; p6l =[]; p7l = []
    for tup in b:
        p1l.append(tup[0]); p2l.append(tup[1]); p3l.append(tup[2]); p4l.append(tup[3]); p5l.append(tup[4]); p6l.append(tup[5]); ; p7l.append(tup[6])
    return(p1l, p2l, p3l, p4l, p5l, p6l, p7l)    

# call the function on the parameters
valst1 = param_fill(p1, p2, p3, p4, p5, p6, p7)  
#%% #create new paramtrail.nc file and adding vaiables to it
#"settings/SH/t1paramtrial_36h.nc"
paramfile = Dataset("summa_zParamTrial_192hru_V.nc",'w',format='NETCDF3_CLASSIC') #create new paramtrail.nc file
...
#varcheck = Dataset ('summa_zParamTrial_144hru.nc')
...
local_atrbt = Dataset("LocalAtributes144hru.nc",'w',format='NETCDF3_CLASSIC')
# define dimensions 
hru = local_atrbt.createDimension('hru', 144) 
...
c3 = np.repeat(newgru[:,np.newaxis], 144, axis=1); newlad = c3.reshape(144,)
...
local_atrbt.close()
#%%
laCheck = Dataset('LocalAtributes144hru.nc')
...
in_condi = Dataset("InitialCond144hru.nc",'w',format='NETCDF3_CLASSIC')
#print ic.variables.keys()
...
hrud = in_condi.createDimension('hru', 144)
# define variables
...
for varname in icif.variables.keys():
    ...
    cic = np.repeat(var[:,np.newaxis], 144, axis=1); newic = cic.reshape(infovar.shape[0],144)
    in_condi.variables[varname][:]=newic
print in_condi.variables['nSoil'][:]

in_condi.close()
#%%
iccheck = Dataset("InitialCond144hru.nc")
