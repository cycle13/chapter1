...
obs_swe = pd.DataFrame (swe_mm, columns=['swe_kg/m2'])
obs_swe.set_index(pd.DatetimeIndex(date_swe),inplace=True)

max_swe_obs = max(obs_swe['swe_kg/m2'])
max_swe_date_obs = obs_swe[obs_swe ['swe_kg/m2']== max_swe_obs].index.tolist()    
#%% Snow depth data
...
counterobs = pd.DataFrame(np.arange(0,np.size(snowdepth_obs_df['observed snowdepth'])),columns=['counter'])
counterobs.set_index(snowdepth_obs_df.index,inplace=True)
snowdepth_obs_df2 = pd.concat([counterobs, snowdepth_obs_df], axis=1)
dosd_obs = [6224]

#%%
hruidxID = list(np.arange(101,125))
hru_num = np.size(hruidxID)
out_names = ['density']# 'Avwp2e','Avas2l','Avas3m','Avce2s','Avtc2s','Avtc3t','Avtc4m','Avns2a','Avns3p','Avns4c']
...
av_ncfiles = ["SA1/density_swampAngel_2007-2008_senatorVariableDecayRate_1.nc"]
...
counter = pd.DataFrame(np.arange(0,np.size(av_sd_df['density101'])),columns=['counter'])
...
counter = pd.DataFrame(np.arange(0,np.size(av_swe_df['density101'])),columns=['counter'])
...
for q in range (np.size(out_names)):
    zerosnowdate_residual.append((first_zerosnowdate_df[out_names_df[0][q]] - dosd_obs[0])/24)
...
x = list(np.arange(1,25))
fig = plt.figure(figsize=(20,15))
plt.bar(x,zerosnowdate_residual_df['density'])
plt.title('residual_dosd', fontsize=42)
plt.xlabel('parameters')
plt.ylabel('day of snow disappreance')
plt.savefig('dosd_density')
...
swe_obs2008 = obs_swe['swe_kg/m2']['2007-12-03 08:00':'2008-06-08 08:00']

fig = plt.figure(figsize=(20,15))
plt.plot(av_swe_df2['density119'])
plt.plot(av_swe_df2['density120'])
plt.plot(av_swe_df2['density121'])
plt.plot(av_swe_df2['density122'])
plt.plot(av_swe_df2['density123'])
plt.plot(swe_obs2008, 'ok')#, linewidth=1)
plt.legend(['swe1','swe2','swe3','swe4','swe5','observed_swe']) 
plt.title('swe kg/m2')#, position=(0.04, 0.88), ha='left', fontsize=12)
plt.xlabel('Time 2007-2008')
plt.ylabel('swe (mm)')
#plt.show()
plt.savefig('swe_dens2.png')
#%%
fig = plt.figure(figsize=(20,15))

plt.plot(av_sd_df2['density119'])
plt.plot(av_sd_df2['density120'])
plt.plot(av_sd_df2['density121'])
plt.plot(av_sd_df2['density122'])
plt.plot(av_sd_df2['density123'])
plt.plot(snowdepth_obs_df ['observed snowdepth'][:], 'k')

plt.legend(['sd1','sd2','sd3','sd4', 'sd5', 'observed_sd'])  #'swe1','swe2','swe3','observed_swe', 
plt.title('snowdepth (m)')#, position=(0.04, 0.88), ha='left', fontsize=12)
plt.xlabel('Time 2007-2008')
plt.ylabel('sd (m)')
#plt.show()
plt.savefig('Sd_dens2.png')
#%%
snow_density = []
for sd in range (len (av_swe_df2['density121'])):
    snow_density.append(av_swe_df2['density121'][sd]/av_sd_df2['density121'][sd])

fig = plt.figure(figsize=(20,15))
plt.plot(snow_density) 
plt.savefig('Density_dens2.png')                      
