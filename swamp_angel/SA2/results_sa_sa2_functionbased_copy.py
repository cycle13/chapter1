...
#%%
height0502M2 = []
for lists in height0502:
    height0502M2.append([2*abs(values) for values in lists])
 
#%%
aaa = [0.5,1.5,2.5,3.25, 4]
ansff=[  1,  1,  1, 0.5, 1]
aaaM2=[2*abs(values) for values in aaa]
sumaaa = []
fff = []
fff2 = []
#fff.append(aaaM2[0])
for i in range (np.size(aaa)):
    #fff.append(sum(ansff[0:i]))
    fff2.append(aaa[i]-sum(fff2[:]))
    
    #fff.append((aaa[i+1]-fff[i])*2)
