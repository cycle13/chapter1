...
plt.savefig('C:/1UNRuniversityFolder/Dissertation/Chapter 1-Snowmelt/swamp_angel/sa_sa2_vars/sa_sa2_VARs_p12FPM_fTCs/sa_sa2_swe_2c_last.png')
...
plt.savefig('C:/1UNRuniversityFolder/Dissertation/Chapter 1-Snowmelt/swamp_angel/sa_sa2_vars/sa_sa2_VARs_p12FPM_fTCs/sa_sa2_sd_2c_last.png')
...
dosd_residual_df_sum_r = pd.DataFrame(np.reshape(np.array(dosd_residual_df_sum),(len(out_names)*len(dosd_residual_df_sum),)), columns = ['SDD'])
...
dosd_residual_dfmin = abs(dosd_residual_df_sum_r).nsmallest(50,'SDD')
...
maxSWE_residual2007 = (abs(maxSWE1_residual2007)+abs(maxSWE2_residual2007)+abs(maxSWE3_residual2007)+abs(maxSWE4_residual2007))/4.
...
#%%
objective_function = (200*meltRate_residual_df_sum + 20*dosd_residual_df_sum)/2. + maxSWE_residual_df_sum
objective_function2 = (meltRate_residual_df_sum/meltRate_normal + 5*(dosd_residual_df_sum/dosd_normal))/2. + maxSWE_residual_df_sum/maxSWE_normal
objective_function3 = meltRate_residual_df_sum/meltRate_normal + 5* (dosd_residual_df_sum/dosd_normal) + maxSWE_residual_df_sum/maxSWE_normal

of_residual_dfmin = []
of_residual_dfmin_indx = []
for combo_of in out_names:
    of_min = abs(objective_function).nsmallest(20,combo_of)[combo_of]
    of_residual_dfmin.append(pd.DataFrame(of_min))
    of_min_indx3 = of_min.index
    of_residual_dfmin_indx.append(of_min_indx3.values)
...
for hru in range (len(hru_of_r)):
    plt.plot(av_swe_df[hru_of_r[hru]], 'purple', linewidth=4)#'green', 
...
safig.tight_layout()  # otherwise the right y-label is slightly clipped
plt.savefig('C:/1UNRuniversityFolder/Dissertation/Chapter 1-Snowmelt/swamp_angel/sa_sa2_vars/sa_sa2_VARs_p12FPM_fTCs/sa_sa2_swe_best_ob_newP3_20_4.png')
#%%
safig2, saax2 = plt.subplots(1,1, figsize=(30,20))#

for hru4 in range (len(hru_of_r)):#len(hru_names_df[0]),
    plt.plot(av_sd_df[hru_of_r[hru4]], 'violet',linewidth=4)# 
...
safig2.tight_layout()  # otherwise the right y-label is slightly clipped
plt.savefig('C:/1UNRuniversityFolder/Dissertation/Chapter 1-Snowmelt/swamp_angel/sa_sa2_vars/sa_sa2_VARs_p12FPM_fTCs/sa_sa2_sd_bestsdd_newP3_4.png')
