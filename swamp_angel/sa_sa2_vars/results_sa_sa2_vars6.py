...
dosd_residual_df_sum_r = pd.concat([dosd_residual_df_sum[out_names[0]],dosd_residual_df_sum[out_names[1]],dosd_residual_df_sum[out_names[2]]],axis=0)#, columns = ['sum2years'])
...
maxSWE_residual_df_mean_r_perc = pd.concat([maxSWE_residual_df_mean[out_names[0]],maxSWE_residual_df_mean[out_names[1]],
                                            maxSWE_residual_df_mean[out_names[2]]],axis=0)#, columns = ['sum2years'])
maxSWE_residual_df_mean_r_perc.index = hru_names_df[0]

maxSWE_residual_df_perc_thresh = maxSWE_residual_df_mean_r_perc[maxSWE_residual_df_mean_r_perc<=0.1]
...
meltRate_residual2007_perc = (pd.DataFrame(mySubtract(meltingrateAvg_2007,meltingrateAvg_obs2007))).T
meltRate_residual2007_perc = (pd.DataFrame(mySubtract(meltingrateAvg_2007,meltingrateAvg_obs2007))).T

meltRate_residual_df_sum = abs(meltRate_residual2007)+abs(meltRate_residual2008)
